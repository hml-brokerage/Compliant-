...
export class ContractorsController {
  ...
  @Get()
  @ApiOperation({
    summary: "Get all contractors (filtered by user role) with search",
  })
  ...
  findAll(
    ...
    if (isNaN(limitNum) || limitNum < 1 || limitNum > 100) {
      throw new BadRequestException(
        "Invalid limit parameter: must be a positive number between 1 and 100",
      );
    }
  ...
  @ApiResponse({
    status: 200,
    description: "Insurance status retrieved successfully",
  })
  @ApiResponse({ status: 404, description: "Contractor not found" })
  ...
  @ApiQuery({
    ...
    required: true,
    type: String,
    description: "Search query for broker name or email",
  })
  @ApiQuery({
    name: "policyType",
    required: false,
    type: String,
    description: "Policy type (GL, AUTO, UMBRELLA, WC, GLOBAL)",
  })
  @ApiQuery({
    name: "limit",
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description: "Broker search results returned successfully",
  })
  async searchBrokers(
    @Query("q") query: string,
