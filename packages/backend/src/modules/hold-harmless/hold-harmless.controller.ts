...
export class HoldHarmlessController {
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: "Auto-generate hold harmless when COI is approved (internal use)",
  })
  @ApiResponse({
    status: 201,
    description: "Hold harmless generated and sent to subcontractor",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Returns hold harmless details for signing",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Subcontractor signature recorded, GC notified",
  })
  async signSubcontractor(
    @Param("id") id: string,
    @Body() signatureData: { signatureUrl: string; signedBy: string },
  ) {
    return this.holdHarmlessService.processSubcontractorSignature(
      id,
      signatureData,
    );
  }
  ...
  @ApiResponse({
    status: 200,
    description: "GC signature recorded, hold harmless completed",
  })
  async signGC(
    @Param("id") id: string,
    @Body()
  ...
  @ApiResponse({
    status: 200,
    description: "Returns list of hold harmless agreements",
  })
