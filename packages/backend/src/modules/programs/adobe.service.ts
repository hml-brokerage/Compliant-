...
export class AdobeService {
  ...
  async getAccessToken(): Promise<string> {
    ...
    try {
      ...
      throw new BadRequestException(`Adobe authentication failed: ${err.message}`);
    }
  }
  ...
  async sendForSignature(params: {
    ...
    try {
      const payload = {
        fileInfos: [
          {
            transientDocumentId: await this.uploadDocument(params.documentUrl, token),
          },
        ],
        recipientSetInfos: [
          {
            recipientSetMemberInfos: [
              {
                ...
                familyName: params.recipientName.split(' ').slice(1).join(' ') || 'User',
              },
            ],
      ...
      this.logger.error(`Failed to send document for signature: ${err.message}`);
      throw new BadRequestException(`Send for signature failed: ${err.message}`);
    }
  }
   ...
   */
  private async uploadDocument(documentUrl: string, token: string): Promise<string> {
    try {
      ...
      formData.append('File', new Blob([documentBuffer], { type: 'application/pdf' }));
   ...
   */
  private async getSigningUrl(agreementId: string, token: string): Promise<string> {
    try {
      const response = await fetch(`${this.baseUrl}/agreements/${agreementId}/signingUrls`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`,
          'x-api-user': this.organizationId,
      ...
      const data = (await response.json()) as { signingUrlSetInfos?: Array<{ signingUrls?: Array<{ url: string }> }> };
      ...
      throw new BadRequestException(`Failed to get signing URL: ${err.message}`);
    }
  }
  ...
  async getAgreementStatus(agreementId: string): Promise<{
    ...
    try {
      const response = await fetch(`${this.baseUrl}/agreements/${agreementId}`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`,
          'x-api-user': this.organizationId,
      ...
      const data = (await response.json()) as { status: string; documentIds?: string[] };
      ...
      throw new BadRequestException(`Failed to get agreement status: ${err.message}`);
    }
  }
}
